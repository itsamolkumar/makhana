...
import { ReceiptIndianRupee, Truck, Trash2, ShoppingBag } from "lucide-react";
...
export default function CartPage() {
  const { items, subtotal, discount } = useAppSelector((state) => state.cart);
  ...
  useEffect(() => {
    const loadPricingSettings = async () => {
      try {
        ...
        // Cart preview uses defaults if the pricing config is unavailable.
      }
    };
  ...
  const totals = useMemo(
    () => computeOrderTotals(subtotal, discount, pricingSettings),
    [discount, pricingSettings, subtotal]
  );
                  ...
                  <div className="flex flex-col sm:flex-row gap-4 sm:gap-6 relative">
                    ...
                    <div className="flex-1 min-w-0 pr-8 sm:pr-0">
                      ...
                      <div className="flex items-center gap-2 mb-4">
                        <span className="text-lg sm:text-xl font-bold text-[var(--color-primary)]">Rs {price}</span>
                        {item.product.discountPrice && (
                          <span className="text-xs sm:text-sm text-gray-500 line-through">Rs {item.product.price}</span>
                        )}
                      </div>
                      ...
                      <div className="flex flex-wrap items-center gap-2 sm:gap-4">
                        ...
                        <p className="text-xs sm:text-sm text-gray-500">({item.quantity} x Rs {price})</p>
                      </div>
                    </div>
                    ...
                    <div className="flex sm:flex-col items-center sm:items-end justify-between mt-4 sm:mt-0 pt-4 sm:pt-0 border-t sm:border-0 border-gray-100">
                      <div className="text-left sm:text-right">
                        <p className="text-lg sm:text-2xl font-bold text-[var(--heading-color)]">Rs {itemTotal}</p>
                        {item.product.discountPrice && (
            ...
            >
              ...
              <div className="space-y-4 py-6 border-y border-gray-200">
                ...
                <div className="flex justify-between items-center text-gray-700">
                  <span>GST ({(pricingSettings.gstRate * 100).toFixed(0)}%)</span>
                  <span className="font-semibold">Rs {totals.gst.toFixed(2)}</span>
                </div>

                <div className="flex justify-between items-center text-gray-700">
                  <span>Service charge</span>
                  <span className="font-semibold">Rs {totals.serviceCharge.toFixed(2)}</span>
                </div>

                <div className="flex justify-between items-center text-gray-700">
                  <span>Delivery charge</span>
                  <span className="font-semibold">Rs {totals.deliveryCharge.toFixed(2)}</span>
                </div>
              ...
              <div className="space-y-2 bg-gradient-to-br from-[var(--color-primary)]/5 to-[var(--color-accent)]/5 rounded-2xl p-5">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-3xl font-bold text-[var(--color-primary)]">Rs {totals.total.toFixed(2)}</span>
                </div>
                <p className="text-xs text-gray-500 text-center">(Including GST, service, and delivery charges)</p>
              </div>
              ...
              <div className="space-y-3 pt-4 border-t border-gray-200">
                ...
                <div className="flex items-start gap-3 text-sm text-gray-600">
                  ...
                  <span>Fast delivery with transparent shipping charges</span>
                </div>
                <div className="flex items-start gap-3 text-sm text-gray-600">
                  <ReceiptIndianRupee className="w-5 h-5 text-[var(--color-primary)] flex-shrink-0 mt-0.5" />
                  <span>Every payable charge is shown before checkout</span>
                </div>
              </div>
            </motion.div>
          </div>
